<script lang="ts">
	...
	function getPrice(companyId: bigint): Price | undefined {
		return prices?.find((p) => p.companyId === companyId);
	}
	...
	let flatData = $derived.by(() => {
		...
		return companies
			.map((company) => {
				...
				const shares = company.listedShares ? Number(company.listedShares) : 0;
								...
								<Group>
									...
									{#if showValue}
										<Text
											...
											textAnchor="middle"
											verticalAnchor="middle"
											value={formatLarge(node.data.value ?? 0)}
		...
		{#if hoveredNode}
			...
			>
				...
				<div class="mt-1 flex items-center justify-between gap-4">
					...
					<span class="font-medium tabular-nums">{formatLarge(hoveredNode.marketCap)}</span>
				</div>
			</div>
		{/if}
...
{:else}
	<div class="flex h-[400px] items-center justify-center rounded-xl border border-border bg-muted/30">
		<p class="text-sm text-muted-foreground">No market cap data available</p>
	</div>
{/if}
